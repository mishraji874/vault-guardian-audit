...
contract VaultGuardiansBase is AStaticTokenData, IVaultData {
        ...
        returns (address)
    {
        ...
        if (address(token) == address(i_tokenOne)) {
            ...
            new VaultShares(IVaultShares.ConstructorData({
                asset: token,
                // @audit - low - after the use of the token_one_vault_name there will be the use of the token_two_vault_name
                vaultName: TOKEN_ONE_VAULT_NAME,
                // @audit - low- here will be the use of the token_two_vault_symbol
                vaultSymbol: TOKEN_ONE_VAULT_SYMBOL,
                guardian: msg.sender,
                allocationData: allocationData,
                aavePool: i_aavePool,
    ...
    function _becomeTokenGuardian(IERC20 token, VaultShares tokenVault) private returns (address) {
        // @audit-info check for address(0) when assigning values to state variables
        s_guardians[msg.sender][token] = IVaultShares(address(tokenVault));
        emit GuardianAdded(msg.sender, token);
        // @audit - high - it will cause the guardians to mint infinite tokens
